...
void VulkanEngine::init_swapchain()
{
	...
	VkImageViewCreateInfo
		rview_info = vkinit::imageview_create_info(_drawImage.imageFormat, _drawImage.image, VK_IMAGE_ASPECT_COLOR_BIT);
...
{
	VkRenderingAttachmentInfo
		colorAttachment = vkinit::attachment_info(targetImageView, nullptr, VK_IMAGE_LAYOUT_GENERAL);
...
void VulkanEngine::immediate_submit(std::function<void(VkCommandBuffer cmd)>&& function)
{
	...
	VkCommandBufferBeginInfo
		cmdBeginInfo = vkinit::command_buffer_begin_info(VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
...
void VulkanEngine::draw()
{
	...
	_drawExtent.height = std::min(_swapchainExtent.height, _drawImage.imageExtent.height) * m_renderScale;
	_drawExtent.width = std::min(_swapchainExtent.width, _drawImage.imageExtent.width) * m_renderScale;
...
// NOTE : Why SSBOs ?
// SSBO are generic read write buffers, we'll index in them to retrieve our vertex data
GPUMeshBuffers VulkanEngine::uploadMesh(std::span<uint32_t> indices, std::span<Vertex> vertices)
{
	...
	// VK_BUFFER_USAGE_STORAGE_BUFFER_BIT : Because SSBO
	// VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT : Allows us to retrieve address
	...
	// Staging buffer (so we can transfer to GPU only memory)
	AllocatedBuffer staging =
		create_buffer(vertexBufferSize + indexBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);

	// Get the mapped memory address, which means CPU accessible memory
	// you can copy to that will be replicated on the GPU
	...
	// TODO : In future let this use the copy/transfer queue
...
void VulkanEngine::init_default_data()
{
	...
	GLTFMetallic_Roughness::MaterialConstants
		* sceneUniformData = (GLTFMetallic_Roughness::MaterialConstants*)materialConstants.allocation->GetMappedData();
	...
	defaultData = metalRoughMaterial.write_material(_device,
	...
	bool mipmapped)
{
	...
	AllocatedBuffer
		uploadbuffer = create_buffer(data_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine* engine)
{
	...
	{
	  vkDestroyPipelineLayout(engine->_device, opaquePipeline.layout, nullptr); // Both pipelines use the same layout
